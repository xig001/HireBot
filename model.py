...
def recommend_jobs(resume_pdf_path, job_df, glove_embeddings, lambda_pref=0.75, top_k=3):
    # Parse and vectorize resume
    raw_text = parse_pdf(resume_pdf_path)
    resume_info = extract_resume_info("uploaded_resume", raw_text)
    resume_text = " ".join([
        resume_info.get(k, "") for k in ["title", "summary", "skills", "experience", "education"]
    ])
    resume_vec = vectorize_text_glove(resume_text, glove_embeddings)

    # Ensure salary column is available
    if 'salary' not in job_df.columns:
        if 'max_salary' in job_df.columns:
            job_df['salary'] = job_df['max_salary']
        else:
            job_df['salary'] = np.random.randint(30000, 150001, size=len(job_df))

    # Vectorized job features
    ...
    # Return filtered DataFrame with scores
    result_df = job_df.iloc[top_idx].copy()
    result_df["similarity_score"] = similarity_scores[top_idx]
    result_df["salary_score"] = salary_scores[top_idx]
    result_df["combined_score"] = combined_scores[top_idx]

    return result_df, similarity_scores[top_idx], salary_scores[top_idx], combined_scores[top_idx]
